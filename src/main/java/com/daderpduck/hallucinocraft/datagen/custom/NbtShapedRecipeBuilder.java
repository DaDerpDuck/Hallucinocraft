package com.daderpduck.hallucinocraft.datagen.custom;
...
public class NbtShapedRecipeBuilder implements RecipeBuilder {
    ...
    public static class Result implements FinishedRecipe {
        ...
        public Result(ResourceLocation pId, Item pResult, int pCount, String pGroup, List<String> pPattern, Map<Character, Ingredient> pKey, Advancement.Builder pAdvancement, ResourceLocation pAdvancementId, CompoundTag tag) {
            this.id = pId;
            this.result = pResult;
            this.count = pCount;
            this.group = pGroup;
            ...
            if (this.nbt != null) {
                ...
                nbt.getAllKeys().forEach(key -> {
                    switch (nbt.getTagType(key)) {
                        case 1 -> jsonnbt.addProperty(key, nbt.getByte(key));
                        case 2 -> jsonnbt.addProperty(key, nbt.getShort(key));
                        case 3 -> jsonnbt.addProperty(key, nbt.getInt(key));
                        case 4 -> jsonnbt.addProperty(key, nbt.getLong(key));
                        case 5 -> jsonnbt.addProperty(key, nbt.getFloat(key));
                        case 6 -> jsonnbt.addProperty(key, nbt.getDouble(key));
                        case 8 -> jsonnbt.addProperty(key, nbt.getString(key));
                        // too lazy to do the rest
                        default -> throw new IllegalStateException("Unexpected value: " + nbt.getTagType(key));
                    }
                });

                jsonobject1.add("nbt", jsonnbt);
            }
